# 📱 智能手机选购助手

基于官方数据的多维度手机筛选工具，帮助你找到最适合的手机。

## 功能特点

- **230+ 款机型**：覆盖苹果、华为、小米、OPPO、vivo、三星等12个品牌
- **多维度筛选**：品牌、屏幕形态、处理器、特性标签（可多选叠加）
- **丰富标签**：潜望长焦、大电池、轻薄、防水、NFC、红外、USB3.0、无线充电、散热风扇等
- **多种排序**：最新发布、电池容量、重量、屏幕大小、快充功率
- **详细参数**：处理器、屏幕、电池、充电、USB版本、重量、系统、摄像头等
- **数据准确**：所有数据来自各品牌官网，截止到2026年5月6日

## 访问方式

直接访问 GitHub Pages 链接：[https://你的用户名.github.io/phone-selector/](https://你的用户名.github.io/phone-selector/)

## 本地使用

1. 下载 `index.html` 文件
2. 用浏览器打开即可使用
3. 无需安装任何依赖，纯前端实现

## 数据来源

- 苹果官网 (apple.com.cn)
- 华为官网 (huawei.com) / 荣耀商城 (vmall.com)
- 小米官网 (mi.com)
- OPPO官网 (oppo.com)
- vivo官网 (vivo.com.cn)
- 三星官网 (samsung.com.cn)
- 荣耀官网 (honor.com)
- 一加官网 (oneplus.com)
- 真我官网 (realme.com)
- iQOO官网 (iqoo.com)
- 红魔官网 (nubia.com)
- REDMI官网 (redmi.com)

## 更新日志

### 2026-05-07
- 初始版本发布
- 包含230+款机型数据
- 支持多维度筛选和排序
- 默认按最新发布时间排序

## License

MIT License