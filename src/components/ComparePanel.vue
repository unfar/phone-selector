...
const fields = [
  ...
  { l: '📸 后置摄像头', v: p => { const s = getCameraSpecs(p); const r = s.find(x => x.l === '后置'); return r ? r.v.replace(/\n/g, ' | ') : '—' } },
  { l: '🤳 前置摄像头', v: p => { const s = getCameraSpecs(p); const r = s.find(x => x.l === '前置'); return r ? r.v : '—' } },
